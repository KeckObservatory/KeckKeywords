...
class keyword_server_handler (EasyHTTPHandler):
    """
    ...
    def show(self, req, qstr):
        ...
        return json.dumps({'server':self._http_server, 'keyword':self._http_keyword, 'value':value}), self.jsonText
...
def start_http_server():
    port = 5002
    keyword_server_handler.DocRoot = "."
    keyword_server_handler.defaultFile = ""
    httpd = EasyHTTPServerThreaded (("", port), keyword_server_handler)
    hostname = socket.gethostname()
    print("HTTPD started %s %d" % (socket.gethostbyaddr(socket.gethostbyname(hostname)), port))
    print("DocRoot is " +  keyword_server_handler.DocRoot)
